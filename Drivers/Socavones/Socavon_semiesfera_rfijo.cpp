...
public:
  ...
  */
    ...
    {
        float t0 = 0.03; //tiempo de espera inicial
        float dt = 0.05; //tiempo de espera entre remociones de particulas
        static int count = 0, maxcount = t0 / (tc / 50);
        if (count > maxcount)
        {
            count = maxcount - dt / (tc / 50);
            //int Pselec = round(random.getRandomNumber(0,particleHandler.getNumberOfObjects()));
            for (unsigned int i = 0; i < particleHandler.getNumberOfObjects();i++)
            {
                //double Pselec = random.getRandomNumber(0, 1);
                if (pow(pow(particleHandler.getObject(i)->getPosition().X, 2) + pow(particleHandler.getObject(i)->getPosition().Y, 2) + pow(particleHandler.getObject(i)->getPosition().Z, 2), 0.5) < Resf)
                {
...
int main(int argc, char *argv[])
{
    ...
    problem.setNumberofParticles(7e3);
    // Set domain size
