...
public:
 ...
 void setupInitialConditions() override
 {
    ...
    double radius = 0.8e-3; //radio particulas
   ...
   //Add your walls below, and don't forget to set the species!
    ...
    //makeDrum();
    double drumRadius = getXMax();
    double Hdrum = getZMax();
	wallHandler.clear();
    ...
    while (numInserted < Np)
	    {
            ...
            do
            {
                ...
                R = random.getRandomNumber(3*radius,drumRadius-radius); // Find a random radial position within the cylinder
                ...
                p0.setPosition(Vec3D(R*cos(C),R*sin(C),random.getRandomNumber(2*radius+getZMin(),getZMax()-radius)));
   ...
   std::cout << "Number of particles inserted = " << numInserted << std::endl;
    ...
    if (count > maxcount)
    {
        ...
        for (unsigned int i = 0; i < particleHandler.getNumberOfObjects();)
        {
            ...
            if (particleHandler.getObject(i)->getPosition().Z  > -0.035 && particleHandler.getObject(i)->getPosition().Z  < -0.03 && particleHandler.getObject(i)->getPosition().X  < 0.01 && particleHandler.getObject(i)->getPosition().X  > -0.01)
            {
...
int main(int argc, char *argv[])
{
    ...
    //fullName = nombre.str();
    problem.setName("Socavon_cilindro");
    //**************STL Filename*******************
