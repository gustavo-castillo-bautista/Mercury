...
public:
 ...
 void setupInitialConditions() override
 {
   ...
   //Add your walls below, and don't forget to set the species!
    ...
    w1.set(Vec3D(1.0, 0.0, 0.0), Vec3D(getXMax(), 0.0, 0.0));
    wallHandler.copyAndAddObject(w1);
        
    w1.set(Vec3D(-1.0, 0.0, 0.0), Vec3D(getXMin(), 0.0, 0.0));
    wallHandler.copyAndAddObject(w1);
        
    w1.set(Vec3D(0.0, 1.0, 0.0), Vec3D(0.0, getYMax(), 0.0));
    wallHandler.copyAndAddObject(w1);
        
    w1.set(Vec3D(0.0, -1.0, 0.0), Vec3D(0.0, getYMin(), 0.0));
    wallHandler.copyAndAddObject(w1);
...
int main(int argc, char *argv[])
{
    ...
    problem.setNumberofParticles(1.0e4);
    // Set domain size
    ...
    //problem.setInteractionsWriteVTK(FileType:: ONE_FILE);
