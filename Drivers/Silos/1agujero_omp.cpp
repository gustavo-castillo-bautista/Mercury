...
int main(int argc, char *argv[])
{
    ...
    int Lbox = helpers::readFromCommandLine(argc, argv, "-L",45);
    int nthreads = helpers::readFromCommandLine(argc, argv, "-omp",1);
    int D = helpers::readFromCommandLine(argc, argv, "-D",6);
    int run = helpers::readFromCommandLine(argc, argv, "-run",100);

    //*******************Setting Up Filename******************************************
    std::stringstream nombre;
    std::string fullName;
    nombre << "D" << D << "mm_L" << Lbox << "mm_run" << run;
    fullName = nombre.str();
    ...
    problem.setTimeMax(6.0);
    problem.setSaveCount(2500);
    problem.setWaitingTime(0.18);
    ...
    problem.setNumberOfOMPThreads(nthreads);
