...
public:
 ...
 void setupInitialConditions() override
 {
   ...
   //Add your walls below, and don't forget to set the species!
    ...
    wallHandler.readTriangleWall("CAD/placa_2agujerosv4.stl",speciesHandler.getObject(1), scaleFactor,shift,velocity,angularVelocity);
    ...
    // InfiniteWall w1;
    // w1.set(Vec3D(0,0,1), Vec3D(0,0,Hdrum)); //Top wall
    ...
    PeriodicBoundary b1;
    //PeriodicBoundary b2;
    b0.set(Vec3D(1.0, 0.0, 0.0), -0.02, 0.02);
    boundaryHandler.copyAndAddObject(b0);
    b1.set(Vec3D(0.0, 1.0, 0.0), -0.02, 0.02);
    ...
    while (numInserted < Np)
	    {
            ...
            do
            {
                ...
                Rx = random.getRandomNumber(radius-0.02,0.02-radius); // Find a random radial position within the cylinder
                Ry = random.getRandomNumber(radius-0.02,0.02-radius); // Find a random radial position within the cylinder
                Rz = random.getRandomNumber(radius+getZMin(),getZMax()-radius); // Find a random radial position within the cylinder
...
int main(int argc, char *argv[])
{
    ...
    problem.setNumberofParticles(8e3);
    // Set domain size
