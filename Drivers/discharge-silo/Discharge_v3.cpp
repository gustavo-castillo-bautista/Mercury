...
public:
 ...
 void setupInitialConditions() override
 {
   ...
   //Add your walls below, and don't forget to set the species!
    ...
    w.addObject(Vec3D(1,0,0), Vec3D(drumRadius,0,0));
    ...
    while (numInserted < Np)
	    {
            ...
            do
            {
                ...
                p0.setPosition(Vec3D(R*cos(C),R*sin(C),random.getRandomNumber(-Hdrum/2,Hdrum/2)));
   ...
   std::cout << "Number of particles inserted = " << numInserted << std::endl;
    ...
    if (count > maxcount)
    {
        ...
        for (unsigned int i = 0; i < particleHandler.getNumberOfObjects();)
        {
            ...
            if (particleHandler.getObject(i)->getPosition().Z  < -0.038)
            {
...
int main(int argc, char *argv[])
{
    ...
    // Set domain size
    problem.setMax({0.04,0.04,0.04});
    problem.setMin({-0.04,-0.04,-0.04});
